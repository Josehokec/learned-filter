...
class Data(object):
    ...
    def random_shuffle(self):
        np.random.shuffle(self.positives)
...
def split_negatives(data, train_portion=0.9):
    ...
    s2: for testing
    """
    print('divide ratio is: ', train_portion)
...
def vectorize_dataset(text_X, text_y, maxlen):
    ...
    function: string -> vector<int>
    eg. www.baidu.com -> [x1, x1, x1, x2, x3, x4, x5, x6, x7, x2, x8, x9, x10]
    """
...
def show_distribute(p_scores, n_scores, save_dir=None):
    """
    :param p_scores: positives scores
    :param n_scores: negatives scores
    :param save_dir: figure save directory, default is current-time.png
    """
    plt.style.use('seaborn-deep')
    bins = np.linspace(0, 1, 21)
    plt.hist([p_scores, n_scores], bins, label=['p', 'n'])
...
def numpy_to_excel(filename, np_data):
    """
    function: store data to excel
    :param filename: excel filename
    :param np_data: need to store data
    """
    excel_data = pd.DataFrame(np_data)
